...
Encrypted, local-first notes app built with Expo SDK 56. **v1.2.13** fixes Google Play photo policy compliance (system photo picker, no broad media access), removes gallery scrub, and adds the app icon on Home — plus SoFo Postpartum HV, encrypted voice memos, Totus Assist, Note Assist, secure attachments, Template Studio AI, trip planner, and read-only web vault at https://totus--notes.web.app/vault.
